...
import de.sightly_robot.sightly_robot.visual.core.handler.MqttHandler.MqttProtocol;

/**
 * Handles arguments passed through the console.
 * 
 * Uses Apache-commons-CLI to parse and store options received via command line.
 * 
 * @author Rico Schrage
 */
public class CommandLineHandler {
	...
	/** default IP which will be used when the IP flag is not set */
	private static final String DEFAULT_IP = "127.0.0.1";
	/** Rege, which checks if the passed IP is valid */
	...
	/**
	 * description of the application, which will appear as header when printing
	 * usage information
	 */
	private static final String HEADER = "Starts a Visualization of the robot game.";
	/**
	 * description of the application, which will appear as footer when printing
	 * usage information
	 */
	...
	/**
	 * Constructs a {@link CommandLineHandler}.
	 */
	 ...
	 * 
	 * Cancels parsing when finding the help flag.
	 * 
	 * @param args
	 *            arguments as array
	...
	public boolean parse(String[] args) {
		try {
			...
			if (!brokerIp.matches(IP_REGEX)) {
				brokerIp = DEFAULT_IP;
				log.warn(
						"Given broker IP doesn't have the correct format. {} will be used instead.",
						brokerIp);
			}
			options.put(OptionKey.IP,
					brokerIp.replace("tcp://", "").replace("ssl://", ""));
	...
	private static Options createOptions() {
		Option help = Option.builder(HELP_SHORT).required(false)
				.longOpt(HELP_LONG).desc("Prints usage information.").build();
		...
		Option proto = Option.builder(PROTO_SHORT).required(false)
				.longOpt(PROTO_LONG).type(String.class).hasArg()
				.desc("The transfer protocol of the mqtt connection.").build();

		Option windowed = Option.builder(WINDOW_SHORT).required(false)
				.longOpt(WINDOW_LONG)
				.desc("Indicates that you want to start without decoration.")
				.build();

		Option fullscreen = Option.builder(FULLSCREEN_SHORT).required(false)
				.longOpt(FULLSCREEN_LONG).hasArg()
				.desc("Enables fullscreen on startup").build();

		return new Options().addOption(help).addOption(debug).addOption(ip)
				.addOption(proto).addOption(windowed).addOption(fullscreen);
	}
