...
/**
 * The interface describes a combination of a non-persistent and a persistent
 * way to manage several options. It uses an internal
 * {@link com.badlogic.gdx.Preferences} object to handle the data, which should
 * be saved in a persistent way. Furthermore there are several internal map to
 * store settings non-persistent. But the way to access these settings is
 * identical. Important: You can't save non-persistent data with a key, which
 * was already used for persistent data, otherwise you will get an exception.
 * 
 * @author Rico Schrage
 */
public interface IPreferences<T extends IPreferencesKey> extends IPreferencesObservable<T> {
	...
	/**
	 * Returns the boolean, which is mapped to <code>key</code>. If there is no
	 * value mapped to <code>key</code>, <code>def</code> will be returned.
	 * 
	 * @param key
	 *            key of the value
	 * @param def
	 *            value, which should be returned when there is no value mapped
	 *            to <code>key</code>
	...
	/**
	 * Returns the float value, which is mapped to <code>key</code>. If there is
	 * no value mapped to <code>key</code>, <code>def</code> will be returned.
	 * 
	 * @param key
	 *            key of the value
	 * @param def
	...
	/**
	 * Returns the integer value, which is mapped to <code>key</code>. If there
	 * is no value mapped to <code>key</code>, <code>def</code> will be
	 * returned.
	 * 
	 * @param key
	 *            key of the value
	...
	/**
	 * Returns the string, which is mapped to <code>key</code>. If there is no
	 * value mapped to <code>key</code>, <code>def</code> will be returned.
	 * 
	 * @param key
	 *            key of the value
	 ...
	 * 
	 * @param key
	 *            - the key, the value should be mapped to
	 * @param value
	...
	/**
	 * Puts given <code>value</code> in the map. Depending on
	 * <code>persistent</code> the value will be stored
	 ...
	 *            - the value, which should be stored
	 * @param persistent
	 *            - if true the value will be stored in a persistent way
	 */
