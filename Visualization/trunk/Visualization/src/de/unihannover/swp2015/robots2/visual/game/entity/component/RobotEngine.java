...
import de.unihannover.swp2015.robots2.visual.util.TransformUtil;
...
public class RobotEngine extends Component {
	...
	/**
	 * Called when an update occurs and the robot is not visible.
	 * 
	 * @param newRenderX new x position
	 * @param newRenderY new y position
	 * @param orientation new orientation
	 */
	private void spawn(float newRenderX, float newRenderY, Orientation orientation) {
	 ...
	 * @param robo robot to be updated
	 * @return x as virtual screen coordinate
	 */
	private float calculateNewX(Orientation orientation, float progress, IRobot robo) {
		...
		final float newRenderX = fieldXOld + factorX * Math.abs(fieldXOld - fieldXNew) * progress;	
		return newRenderX;
	}
	 ...
	 * @return x as virtual screen coordinate
	 */
	private float calculateNewY(Orientation orientation, float progress, IRobot robo) {
		...
		final float newRenderY = fieldYOld + factorY * Math.abs(fieldYOld - fieldYNew) * progress;
		return newRenderY;
	}
